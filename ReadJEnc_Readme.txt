...
※NuGetからインストールされるパッケージには、DLLのみ入っています。
...
(9)文字コード自動判別オブジェクトを切り替えた場合、
　 　ShiftJIS/EUC-JPの代わりに以下の文字コードについて判別
...
◎ライブラリをVisualStudioのプロジェクトに組み込みます。
 ...
 (2)Vectorからダウンロードしたzipファイルを利用する場合
 ...
 ・ReadJEncのGitHubリポジトリ [ https://github.com/hnx8/ReadJEnc ]より
   CharCode.cs／FileType.cs／FileReader.cs／ReadJEnc.csの４ファイルを取得し、
   Visual Studioプロジェクトに追加してください。
...
◎Byte配列の内容から文字コードを判別する場合は、
  ...
  HttpClientクラスでWebページの文字コードを推定する具体的なサンプルが
...
「ReadJEncSample.exe」が、実際にライブラリを使用したサンプルです。
 （ロジック部100行ちょっとの小さなアプリケーションです。ソースも同梱しています）
...
【４】ライブラリの仕様について
 ...
 (2) ReadJEnc.cs   : 文字コード自動判別処理クラス本体
    ...
    ⇒バイト配列から文字コード判別を行う機能を提供します。
      （各国語の文字コード判定用内部クラス：SJIS/BIG5TW/GB18030/UHCKR/SBCS、
        およびJIS判定支援クラスを内包しています）
      実際に各国語の文字コードを判別するインスタンスは、
      静的フィールドJP/ANSI/TW/CN/KR・CP1250～CP1258・TIS620として
      定義されています。
